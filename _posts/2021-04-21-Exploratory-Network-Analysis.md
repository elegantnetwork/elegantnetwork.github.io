...
Suzieq is using standard data science tools like Python, Pandas, and Parquet. We’ve built network engineering and operations applications on top of that, but you can use a [Jupyter notebook](https://jupyter.org/) and do all the analysis yourself if you so desire. Jupyter is the standard interface for datascience, so it can allow you to ask any question you can image. In Suzieq, in both the GUI and the CLI, we've designed it to be specific to networking, allowing a more intuitive approach to exploring your network. 
...
![Suzieq bgp show eBGP](/assets/images/2021-04-xna/bgp-show-eBGP.png) This shows, as expected, all the eBGP peers are to/from the dcedge and firewall devices.
