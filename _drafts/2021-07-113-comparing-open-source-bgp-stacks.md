...
The stacks evaluated are [BIRD](https://bird.network.cz/), [FRRouting](https://frrouting.org/), [GoBGP](https://github.com/osrg/gobgp). These have different sets of features, including FRR and BIRD have more full routing stacks. BIRD and FRRouting are single process/core stacks while gobgp can use multiple cores. **I was hoping that we'd see the benefits of multiple cores.**
...
bgperf creates all the config in memory before writing it out, so if you have a large number of routes, like 10M, it will run out of memory on the 32bit Python process before it even gets to any of the rest of the test. This clearly could be improved, but I didn't get that far.
...
This test isn't much of a stressor on these stacks so we don't see much differentiation.  In this case, they all  take 1-3seconds for the test. GoBGP takes longer to establish neighbors and it uses a lot (1160 vs 10-20) more CPU. 
...
I wanted to test something crazy, but it failed. the bgperf process runs out of memory creating configs before it even gets stared. Something to be fixed for later.
